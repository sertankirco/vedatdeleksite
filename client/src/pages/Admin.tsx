...
import { BarChart3, FileText, Video, ShoppingBag, Settings, Lock } from "lucide-react";
...
export default function Admin() {
  ...
  const { t } = useLanguage();
  const [activeTab, setActiveTab] = useState<'dashboard' | 'blog' | 'videos' | 'products' | 'settings'>('dashboard');
  ...
  const tabs = [
    { id: 'dashboard' as const, label: t('admin.dashboard'), icon: <BarChart3 className="w-4 h-4" /> },
    { id: 'blog' as const, label: t('admin.blog'), icon: <FileText className="w-4 h-4" /> },
    { id: 'videos' as const, label: t('admin.videos'), icon: <Video className="w-4 h-4" /> },
    { id: 'products' as const, label: t('admin.products'), icon: <ShoppingBag className="w-4 h-4" /> },
    { id: 'settings' as const, label: t('admin.settings'), icon: <Settings className="w-4 h-4" /> },
  ];
            ...
            <div className="space-y-6">
              ...
              <Card className="p-6 bg-gradient-to-r from-blue-50 to-purple-50 dark:from-slate-800 dark:to-slate-900 border-blue-200 dark:border-slate-700">
                <div className="grid md:grid-cols-2 gap-6 items-center">
                  <div>
                    <h3 className="text-xl font-bold mb-2 text-slate-900 dark:text-white">Vedat Delek Astroloji Platformu</h3>
                    <p className="text-slate-700 dark:text-slate-300 mb-4">
                      Unlu astrolog Vedat Delek'in yapay zeka destekli astroloji platformu. Gunluk burc yorumlari, kisisel analiz ve yasam rehberligi hizmetleri sunuyoruz.
                    </p>
                    <p className="text-sm text-slate-600 dark:text-slate-400">Yonetim panelini kullanarak tum icerigi yonetebilirsiniz.</p>
                  </div>
              ...
              <div className="grid md:grid-cols-4 gap-6">
                {[
                  ...
                  <Card key={idx} className="p-6 text-center bg-slate-50 dark:bg-slate-800">
